...
const links = [
  ...
  },
  {
    ...
    subLinks: [{ name: "View All Result", href: "/dashboard/result/view-result" }],
  },
  {
    name: "Guideline",
    icon: Book,
...
export function DashboardSidebar() {
  ...
  return (
    <aside className="w-64 bg-[#1f4f43] border-r border-emerald-900/20 hidden md:flex flex-col shadow-lg">
      <Link href="/dashboard">
      ...
      <nav className="flex-1 p-4 space-y-2 text-emerald-100">
        {links.map((link) => {
            ...
            return (
              <div key={link.name}>
                <button
                  ...
                  className={clsx(
                    "flex items-center justify-between w-full gap-3 p-3 rounded-lg transition hover:bg-emerald-700/50 hover:text-white",
                ...
                {isOpen && (
                  <div className="mt-1 ml-4 pl-4 border-l-2 border-emerald-300 space-y-1 bg-emerald-900/20 rounded-r-md py-2">
                    {link.subLinks.map((subLink) => (
                      <Link
                        key={subLink.href}
                        href={subLink.href}
                        className={clsx(
                          "block p-2 rounded-md transition hover:bg-emerald-700/40 hover:text-white text-sm",
                          pathname === subLink.href && "font-bold text-white"
                        )}
                      >
                        {subLink.name}
                      </Link>
          ...
          return (
            <Link
              ...
              href={link.href}
              className={clsx(
                "flex items-center gap-3 p-3 rounded-lg transition hover:bg-emerald-700/50 hover:text-white",
                active &&
                  "bg-gradient-to-r from-emerald-500 to-green-600 text-white font-semibold shadow-md"
              )}
            >
              <Icon className="w-5 h-5" />
              {link.name}
